...
public class Face {
  ...
  static {
    ...
    HALF_ELLIPSE_BIG = Utils
        .allocateFloats( (Settings.BIG_ELLIPSE_SLICES + 2) * 3 );
    Utils.generateEllipsoid( HALF_ELLIPSE_BIG, 0, Settings.BIG_ELLIPSE_SLICES,
        0f, 0f, 0f, 2f, 2f, 0d, Math.PI );
    HALF_ELLIPSE_BIG.position( 0 );
    
    HALF_ELLIPSE_SMALL = Utils
        .allocateFloats( (Settings.SMALL_ELLIPSE_SLICES + 2) * 3 );
    Utils.generateEllipsoid( HALF_ELLIPSE_SMALL, 0,
        Settings.SMALL_ELLIPSE_SLICES, 0f, 0f, 0f, 2f, 2f, 0d, Math.PI );
    HALF_ELLIPSE_SMALL.position( 0 );
    
    CIRCLE_SMALL = Utils
        .allocateFloats( (2 * Settings.SMALL_ELLIPSE_SLICES + 2) * 3 );
    Utils
        .generateEllipsoid( CIRCLE_SMALL, 0, 2 * Settings.SMALL_ELLIPSE_SLICES,
            0f, 0f, 0f, 2f, 2f, 0d, 2d * Math.PI );
  ...
  // model matrices for the various components -- relative to the face in
  // [-1,1]^^2
  Transform mFaceTop;
  ...
  public void init() {
    mShaderProgram = Utils.loadProgram( Utils.DEFAULT_VERTEX_SHADER,
        Utils.DEFAULT_FRAGMENT_SHADER );
  }
  ...
  public void resize( int width, int height ) {
    ...
    if ( width > height ) {
      ...
      Matrix.frustumM( mProjectionMatrix, 0, -ratio * outer, ratio * outer,
          -outer, outer, 2.9f, 7f );
    } else {
      float ratio = (float) height / (float) width;
      Matrix.frustumM( mProjectionMatrix, 0, -outer, outer, -ratio * outer,
          ratio * outer, 2.9f, 7f );
    }
    Matrix.setLookAtM( mViewMatrix, 0, 0, 0, 3, 0f, 0f, 0f, 0f, 1.0f, 0.0f );
  }
  
  public void setFace( Affect affect, Features features, double lookX,
      double lookY ) {
    ...
    for ( int i = 0 ; i < Settings.NUM_TEETH ; i++ ) {
      mTeethTops[ i ].setTranslation( fx - tx + (float) (i * tw), my - mt - 2f
          * tv + bmh, 0f );
      mTeethBottoms[ i ].setTranslation( fx - tx + (float) i * tw, my - mt + 2f
          * tv - bmh, 0f );
    }
  }
  ...
  public void draw() {
    ...
    Utils.drawEllipsoid( HALF_ELLIPSE_BIG, mShaderProgram, mFaceTop,
        ...
        Settings.FACE_COLOR2, Settings.FACE_COLOR3, mOffsetX + 0.3f * mRadius,
        mOffsetY + 0.7f * mRadius, mRadius );
    Utils.drawEllipsoid( QUAD, mShaderProgram, mFaceMiddle, mViewMatrix,
        ...
        Settings.FACE_COLOR3, mOffsetX + 0.3f * mRadius, mOffsetY + 0.7f
            * mRadius, mRadius );
     ...
     * Utils.drawEllipsoid( HALF_ELLIPSE_BIG, mShaderProgram, mFaceTop,
     * mViewMatrix, mProjectionMatrix, Settings.FACE_COLOR1,
     * Settings.FACE_COLOR2, Settings.FACE_COLOR3, 0.5f * mWidth, 0.5f *
     * mHeight, mRadius ); Utils.drawEllipsoid( QUAD, mShaderProgram,
     * mFaceMiddle, mViewMatrix, mProjectionMatrix,Settings.FACE_COLOR1,
     * Settings.FACE_COLOR2, Settings.FACE_COLOR3, 0.5f * mWidth, 0.5f *
     * mHeight, mRadius ); Utils.drawEllipsoid( HALF_ELLIPSE_BIG,
     * mShaderProgram, mFaceBottom, mViewMatrix,
     * mProjectionMatrix,Settings.FACE_COLOR1, Settings.FACE_COLOR2,
     * Settings.FACE_COLOR3, 0.5f * mWidth, 0.5f * mHeight, mRadius );
     */
    
    // from now on draw to stencil buffers in XOR mode
    
    // TODO use different shaderprogram (no gradients) for drawing to stencil
    // buffer?
    ...
    Utils.drawEllipsoid( CIRCLE_SMALL, mShaderProgram, mEyeLeft, mViewMatrix,
        mProjectionMatrix, Settings.EYE_COLOR1 );
    Utils.drawEllipsoid( CIRCLE_SMALL, mShaderProgram, mEyeRight, mViewMatrix,
        mProjectionMatrix, Settings.EYE_COLOR1 );
    
    // draw mouth (stencilbuffer)
    Utils.drawEllipsoid( HALF_ELLIPSE_BIG, mShaderProgram, mMouthTop,
        mViewMatrix, mProjectionMatrix, Settings.MOUTH_COLOR );
    Utils.drawEllipsoid( HALF_ELLIPSE_BIG, mShaderProgram, mMouthBottom,
        mViewMatrix, mProjectionMatrix, Settings.MOUTH_COLOR );
    
    // from now on, draw in the color buffer but clip to the mask in stencil
    // buffer
    ...
    // draw eyes
    Utils.drawEllipsoid( CIRCLE_SMALL, mShaderProgram, mEyeLeft, mViewMatrix,
        mProjectionMatrix, Settings.EYE_COLOR1, Settings.EYE_COLOR2,
        Settings.EYE_COLOR3, 0, 0, 0 );
    Utils.drawEllipsoid( CIRCLE_SMALL, mShaderProgram, mEyeRight, mViewMatrix,
        mProjectionMatrix, Settings.EYE_COLOR1, Settings.EYE_COLOR2,
        Settings.EYE_COLOR3, 0, 0, 0 );
    ...
    Utils.drawEllipsoid( CIRCLE_SMALL, mShaderProgram, mIrisLeft, mViewMatrix,
        mProjectionMatrix, Settings.IRIS_COLOR1, Settings.IRIS_COLOR2,
        Settings.IRIS_COLOR3, 0, 0, 0 );
    Utils.drawEllipsoid( CIRCLE_SMALL, mShaderProgram, mIrisRight, mViewMatrix,
        mProjectionMatrix, Settings.IRIS_COLOR1, Settings.IRIS_COLOR2,
        Settings.IRIS_COLOR3, 0, 0, 0 );
    Utils.drawEllipsoid( CIRCLE_SMALL, mShaderProgram, mPupilLeft, mViewMatrix,
        mProjectionMatrix, Settings.PUPIL_COLOR );
    Utils.drawEllipsoid( CIRCLE_SMALL, mShaderProgram, mPupilRight,
        mViewMatrix, mProjectionMatrix, Settings.PUPIL_COLOR );
    
    // draw teeth
    for ( int i = 0 ; i < Settings.NUM_TEETH ; i++ ) {
      Utils.drawEllipsoid( QUAD, mShaderProgram, mTeethTops[ i ], mViewMatrix,
          mProjectionMatrix, Settings.TEETH_COLOR );
      Utils.drawEllipsoid( QUAD, mShaderProgram, mTeethBottoms[ i ],
          mViewMatrix, mProjectionMatrix, Settings.TEETH_COLOR );
    }
    ...
    // draw eyebrows
    Utils.drawEllipsoid( QUAD, mShaderProgram, mBrowLeft, mViewMatrix,
        mProjectionMatrix, Settings.BROW_COLOR );
    Utils.drawEllipsoid( QUAD, mShaderProgram, mBrowRight, mViewMatrix,
        mProjectionMatrix, Settings.BROW_COLOR );
    
    // draw lips
    Utils.drawEllipsoid( HALF_ELLIPSE_BIG, mShaderProgram, mMouthTop,
        mViewMatrix, mProjectionMatrix, null, null, Settings.LIP_COLOR, 0f, 0f,
        0f );
    Utils.drawEllipsoid( HALF_ELLIPSE_BIG, mShaderProgram, mMouthBottom,
        mViewMatrix, mProjectionMatrix, null, null, Settings.LIP_COLOR, 0f, 0f,
        0f );
