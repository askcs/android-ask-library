...
/* extends Object */{
  ...
  static public final String DEFAULT_VERTEX_SHADER = "#version 100\n"
      + "uniform mat4 uMVPMatrix;" + "attribute vec4 vPosition;"
      + "void main() {" + "  gl_Position = uMVPMatrix * vPosition;" + "}";
  
  static public final String DEFAULT_FRAGMENT_SHADER = "#version 100\n"
      + "precision mediump float;" + "uniform vec4 vColor1;"
      + "uniform vec4 vColor2;" + "uniform vec2 vGradientPos;"
      + "uniform float vScale;" + "void main() {"
  ...
  static public FloatBuffer generateQuad( FloatBuffer dst, int offset,
    ...
    for ( int i = 0 ; i <= 4 ; i++ ) {
      ...
      vector[ 0 ] = (float) (centerX + Math.sqrt( 0.5d ) * width
          * Math.cos( angle ));
      vector[ 1 ] = (float) (centerY + Math.sqrt( 0.5d ) * height
          * Math.sin( angle ));
      // vertex[ 2 ] = centerZ;
  ...
  static public FloatBuffer generateEllipsoid( FloatBuffer dst, int offset,
      int slices, float centerX, float centerY, float centerZ, float width,
      float height, double alpha, double omega ) {
    float[] vector = new float[] { centerX, centerY, centerZ };
    dst.put( vector, 0, 3 );
  ...
  static public void drawEllipsoid( FloatBuffer ellipse, int program,
    ...
    drawEllipsoid( ellipse, program, model, view, projection, color, color,
        null, 0f, 0f, 1f );
  }
  
  static public void drawEllipsoid( FloatBuffer ellipse, int program,
      Transform model, float[] view, float[] projection, float[] color1,
      float[] color2, float[] color3, float gradientX, float gradientY,
      float radius ) {
