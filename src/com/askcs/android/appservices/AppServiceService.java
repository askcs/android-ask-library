...
public class AppServiceService extends IntentService {
  ...
  public void onCreate() {
    super.onCreate();
    mRestInterface = RestInterface.getInstance( getApplicationContext() );
  };
  
  /**
   * Handles queued intent based on assigned extra parameters inside the intent
   * It is required to send a extra with as "name" INTENT_COMMAND and as "value"
   * one of the actions like INTENT_REGISTER, INTENT_LOGIN etc. Additional
   * parameters can be given by adding the INTENT_EXRA_ extras as extra.
   */
  @Override
  protected void onHandleIntent( Intent intent ) {
    ...
    try {
      ...
      if ( !this.isNetworkOnline() ) {
        resultCode = Errors.ERROR_NO_CONNECTION;
      } else {
        ...
        case INTENT_LOGOUT :
          ...
          Log.i( TAG, "logout: result=" + resultCode );
          break;
        ...
        case INTENT_TRANSMIT_AND_GET_DATA :
          // First transmit the cache and if successful get latest
          // data
        ...
        case INTENT_POST_AFFECT :
          ...
          double arousal = intent.getDoubleExtra( INTENT_EXTRA_AFFECT_AROUSAL,
              0D );
