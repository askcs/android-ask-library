...
public class MessageReceiver extends BaseRestReceiver {
  ...
  private boolean checkMessage( InputStream inputStream ) {
    ...
    Cursor cursor = mAppServiceSqlStorage.selectData( "select "
        + AppServiceSqlStorage.C_MESSAGE_UUID + " from "
        + AppServiceSqlStorage.T_MESSAGE );
    ...
    try {
      ...
      for ( JsonToken token = jParser.nextToken() ; token != JsonToken.END_ARRAY
        ...
        if ( message.getState() == null ) {
          ...
          if ( localItemsList.remove( message.getUuid() )
              || message.getState().equalsIgnoreCase( "SEEN" ) ) {
      ...
      mAppServiceSqlStorage.batchImport( itemsToAdd,
          AppServiceSqlStorage.T_MESSAGE );
      mAppServiceSqlStorage.batchDeleteOnUuid( localItemsList,
          AppServiceSqlStorage.T_MESSAGE );
      ...
      if ( newCount > 0 ) {
        ...
        mContext
            .sendBroadcast( new Intent( "com.ask.moodie.RECEIVED_MESSAGE" ) );
      }
