...
public class RestCache {
  ...
  /**
   * @param contentValues
   * @deprecated Not implemented: put the object into the normal database
   *             instead
   */
  @Deprecated
  ...
  /**
   * @deprecated Not implemented: put the objects into the normal database
   *             instead
   */
  @Deprecated
  ...
  /**
   * Transmits every item in the restCache database trough the rest interface
   * and remove every successfully sent item from the database
   * 
   * @return true if rest cache is empty
   */
  public boolean transmitCache() {
    
    Cursor cacheData = mAppServiceSqlStorage.selectData(
        AppServiceSqlStorage.T_RESTCACHE, null, null, null );
    ...
    if ( cacheData.getCount() > 0 ) {
      ...
      // Get columnIndexes
      int idColumnIndex = cacheData
          .getColumnIndex( AppServiceSqlStorage.C_RESTCACHE_ID );
      ...
      mAppServiceSqlStorage.batchDeleteOnId( idList,
          AppServiceSqlStorage.T_RESTCACHE );
      
      cacheData.close();
      
      cacheData = mAppServiceSqlStorage.selectData(
          AppServiceSqlStorage.T_RESTCACHE, null, null, null );
    ...
    try {
      ...
      do {
        ...
        conn.setRequestProperty( "Cookie",
            "X-SESSION_ID=" + mRestInterface.getXSession() );
