...
public class RestInterface {
    ...
    protected RestInterface(Context context) {
        ...
        mXSession = PreferenceManager.getDefaultSharedPreferences(mContext).getString(
                Prefs.SESSION_ID, "");
     ...
     * @return The session ID, or null if the registration failed
     */
    public int register(String email, String password, String surname, String firstName) {
    ...
    private int registerConnection(String uuid, String password, String name) {
        ...
        try {
            String xSession;
            String urlString = mHost + "/register?" + "uuid=" + URLEncoder.encode(uuid, "UTF-8")
    ...
    public void update(Message message) {
        ...
        try {
            contentValues
                    .put(AppServiceSqlStorage.C_RESTCACHE_CONTENT, message.toJson().toString());

        } catch (JSONException e) {
            e.printStackTrace();
        }
        contentValues.put(AppServiceSqlStorage.C_RESTCACHE_URL, mHost + MessageReceiver.PATH);

        mAppServiceSqlStorage.updateById(message.toContentValues(), AppServiceSqlStorage.T_MESSAGE,
                message.getId());
