...
public class AppServiceSqlStorage extends SqlStorageBase {
  ...
  public void onCreate( SQLiteDatabase db ) {
    ...
    db.execSQL( "create table " + T_MESSAGE + " (" + C_MESSAGE_ID
        ...
        + C_MESSAGE_QUESTION_TEXT + " text," + C_MESSAGE_TYPE + " text,"
        + C_MESSAGE_STATE + " text," + C_MESSAGE_CREATIONTIME + " integer)" );
    
    db.execSQL( "create table " + T_RESTCACHE + " (" + C_RESTCACHE_ID
        + " integer primary key autoincrement," + C_RESTCACHE_ACTION + " text,"
        + C_RESTCACHE_URL + " text," + C_RESTCACHE_CONTENT + " text)" );
  ...
  private static Map<String, AppServiceSqlStorage> instanceMap = new HashMap<String, AppServiceSqlStorage>();
  
  public static AppServiceSqlStorage getInstance( Context context ) {
    return getInstance( context,
        context.getResources().getString( R.string.appservice_host ) );
  }
