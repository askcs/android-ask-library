...
public abstract class SenseTask<T extends Object> extends
  ...
  protected Boolean doInBackground( Map<String, ? super T>... maps ) {
    Log.i( TAG, mSensorName + ": doInBackground: got " + maps.length
        + " arguments" );
    for ( Map<String, ? super T> map : maps ) {
      mApplication.getSensePlatform().addDataPoint( mSensorName, mDisplayName,
          mDescription, getType(), getValue( map ), System.currentTimeMillis() );
    }
