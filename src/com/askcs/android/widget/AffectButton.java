...
public class AffectButton extends GLSurfaceView {
  ...
  public AffectButton( Context context, AttributeSet attrs, int defStyle ) {
    ...
    int c = attrs.getAttributeResourceValue( Constants.ANDROID_NS,
        "background", 0 );
  ...
  public boolean onTouchEvent( MotionEvent event ) {
    ...
    float x = Math
        .max( mOffsetX, Math.min( mOffsetX + mSize - 1, event.getX() ) );
    float y = Math
        .max( mOffsetY, Math.min( mOffsetY + mSize - 1, event.getY() ) );
